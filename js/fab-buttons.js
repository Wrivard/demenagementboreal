...
(function() {
  ...
  const MESSENGER_PAGE_ID = '61555945646998'; // Messenger page ID (update if needed)
  const MESSENGER_URL = 'https://m.me/' + MESSENGER_PAGE_ID; // Universal link (works on all platforms)
  ...
  const buttonMarkup = `
    <a href="${MESSENGER_URL}" class="messenger-button" aria-label="Contactez-nous sur Messenger">
      ${messengerIconSVG}
    </a>
  ...
  function initButtons() {
    ...
    // Bind click handler for Messenger button with mobile detection
    messengerButtons.forEach(function(button) {
      button.addEventListener('click', function(e) {
        // On mobile, try deep link first, then fallback to web
        if (isIOS()) {
          // iOS: Try fb-messenger:// deep link first
          const deepLink = 'fb-messenger://user-thread/' + MESSENGER_PAGE_ID;
          const webLink = MESSENGER_URL;
          
          // Try deep link
          window.location.href = deepLink;
          
          // If deep link fails (app not installed), fallback to web after a delay
          setTimeout(function() {
            // If still on page after 500ms, deep link likely failed
          ...
          // Android: Try intent:// deep link first
          const deepLink = 'intent://user/' + MESSENGER_PAGE_ID + '#Intent;package=com.facebook.orca;scheme=fb-messenger;end';
          const webLink = MESSENGER_URL;
